const categoryConfig = {
  prayer:     { bg: "#1a1025", border: "#4c1d95", dot: "#fbbf24", badgeBg: "#3b0764", badgeText: "#fbbf24", label: "Prayer" },
  fitness:    { bg: "#051a0f", border: "#065f46", dot: "#34d399", badgeBg: "#052e16", badgeText: "#34d399", label: "Fitness" },
  career:     { bg: "#082f49", border: "#0c4a6e", dot: "#38bdf8", badgeBg: "#082f49", badgeText: "#38bdf8", label: "Career" },
  discipline: { bg: "#1a1025", border: "#4c1d95", dot: "#a78bfa", badgeBg: "#1a1025", badgeText: "#a78bfa", label: "Discipline" },
};
    ...
    <div className="max-w-[680px] mx-auto px-4 pb-10">
      ...
      <div className="flex flex-col gap-2">
        {dailySchedule.map((item, i) => {
          ...
          const dayKey = `d${activeDay}-i${i}`;
          const isDone = !!checked[dayKey];
          return (
            <div
              key={i}
              onClick={() => toggle(activeDay, i)}
              className={`flex gap-3 items-start py-3 px-4 rounded-xl bg-[#0e0e18] border border-[#1e1e2e] cursor-pointer transition-all duration-200 ${isDone ? "opacity-55" : "hover:border-[#2a2a3a]"}`}
            >
              {/* Checkbox */}
              <div className={`mt-[2px] w-[18px] h-[18px] rounded border-2 flex items-center justify-center text-[10px] flex-shrink-0 transition-colors duration-200 ${isDone ? "bg-[#7c3aed] border-[#7c3aed] text-white" : "border-[#555] bg-transparent"}`}>
                {isDone && "✓"}
              </div>
